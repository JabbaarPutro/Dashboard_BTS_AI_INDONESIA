(function() {
    ...
    let jaringanFilterTypes = [];
    let layananFilterTypes = [];
    let selectedJaringanFilters = [];
    ...
    const config = {
        ...
        statusOptions: [ { value: 'On Air', text: 'On Air' }, { value: 'Dalam Pembangunan', text: 'Dalam Pembangunan' } ]
    };
    ...
    const btsDashboard = {
        init: async function(map) {
            ...
            try {
                const response = await fetch(`../api/api.php?type=bts`);
                ...
                jaringanFilterTypes = dataFromServer.filters.jaringan || [];
                layananFilterTypes = dataFromServer.filters.jenis_layanan || [];
                selectedJaringanFilters = [...jaringanFilterTypes];
                ...
                console.error('Gagal memuat data BTS:', error);
    ...
    function createCustomIcon(status) {
        const isPrimaryStatus = status === 'On Air';
  ...
  function createTooltipContent(provinceName, data) {
        ...
        <div class="province-tooltip">
            ...
            <div class="tooltip-main-stats">
                <div class="tooltip-total-section">
                    <div class="tooltip-total-label">Total BTS</div>
    ...
    function updatePointMarkers() {
        ...
        allPointData.forEach(point => {
            ...
            const jaringanMatch = selectedJaringanFilters.length === 0 || selectedJaringanFilters.includes(point.jaringan);
            const layananMatch = selectedLayananFilters.length === 0 || selectedLayananFilters.includes(point.jenis_layanan);
            
            if (statusMatch && jaringanMatch && layananMatch) {
                const icon = createCustomIcon(point.status);
                const marker = L.marker([point.lat, point.lon], { icon });
                marker.bindPopup(`
                    <div class="popup-content">
                        <b>${point.nama_situs || 'Nama tidak tersedia'}</b><br>
                        ${point.kabupaten || 'Kabupaten tidak tersedia'}<br>
                        Status: <span class="status-${point.status.replace(/\s+/g, '-').toLowerCase()}">${point.status}</span><br>
                        Jaringan: <span class="network-type">${point.jaringan || 'N/A'}</span><br>
    ...
    function updateFilteredStats() {
        ...
        const filteredPoints = allPointData.filter(p => {
            ...
            const jaringanMatch = (selectedJaringanFilters.length === 0 || selectedJaringanFilters.includes(p.jaringan));
    ...
    function getFilterHtml() {
        return `
            ...
            <div id="jaringan-dropdown" class="relative w-full sm:w-60">
                <button id="jaringan-dropdown-button" type="button" class="relative w-full cursor-default rounded-full bg-white py-2.5 pl-4 pr-10 text-left text-slate-700 shadow-md">
                    <span id="jaringan-selected-text" class="block truncate">Semua Jaringan</span>
                    <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                        <svg class="h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.25 4.25a.75.75 0 01-1.06 0L5.23 8.27a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
                        </svg>
                ...
                <ul id="jaringan-dropdown-options" class="absolute z-20 mt-1 max-h-60 w-full overflow-auto rounded-xl bg-white hidden shadow-lg"></ul>
            </div>
        ...
        filterData.forEach(filter => {
            ...
            li.innerHTML = `
                <label class="flex items-center w-full h-full cursor-pointer">
                    <input type="checkbox" value="${filter}" class="filter-checkbox w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" checked>
    ...
    function attachEventListeners() {
        ...
        statusBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            document.getElementById('jaringan-dropdown-options').classList.add('hidden');
        ...
        createCheckboxFilter('jaringan-dropdown-options', 'jaringan-dropdown-button', 'jaringan-selected-text', jaringanFilterTypes, selectedJaringanFilters, 'Jaringan');
        ...
        window.addEventListener('click', (e) => {
            ...
            if (!e.target.closest('#jaringan-dropdown')) document.getElementById('jaringan-dropdown-options').classList.add('hidden');
            if (!e.target.closest('#layanan-dropdown')) document.getElementById('layanan-dropdown-options').classList.add('hidden');
        });
    }

    window.btsDashboard = btsDashboard;
})();